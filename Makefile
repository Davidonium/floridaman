...
deploy: build-linux ## deploys the application to production, assumes that the config in ~/.ssh/config has a Host floridaman entry
	# copy the binary generated by build target
	scp $(APP_BIN_LINUX) floridaman:/tmp/floridaman

	# reload the systemd service
	ssh floridaman "mv /tmp/floridaman /opt/floridaman/floridaman; sudo systemctl restart floridaman"
